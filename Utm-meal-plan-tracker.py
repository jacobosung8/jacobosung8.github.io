...
class MealPlanEngine:
    ...
    def init_db(self):
        ...
        cursor.execute("""
                       ...
                       (
                           ...
                           basic_balance REAL,
                           flex_balance REAL,
                           stay_weekends INTEGER,
                           include_reading_week INTEGER
                       )
                       """)
    ...
    def delete_transaction(self, tx_id):
        ...
        cursor.execute("SELECT amount, fund_type FROM transactions WHERE id = ?", (tx_id,))
        row = cursor.fetchone()

        if row:
            amount, fund_type = row
            cursor.execute("DELETE FROM transactions WHERE id = ?", (tx_id,))

            # Refund the balance back to the correct fund
            if fund_type == "Basic":
                cursor.execute("UPDATE plan_config SET basic_balance = basic_balance + ? WHERE id = 1", (amount,))
            else:
                cursor.execute("UPDATE plan_config SET flex_balance = flex_balance + ? WHERE id = 1", (amount,))
    ...
    <script>
        async function fetchDashboard() {
            ...
            if (data.transactions.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" style="text-align:center; color:#94a3b8;">No purchases logged yet.</td></tr>';
            } else {
                data.transactions.forEach(tx => {
                    ...
                    tbody.innerHTML += `
                        <tr>
                            <td>
                                <strong>${tx.item}</strong><br>
                                <span style="color:#94a3b8; font-size:0.75rem;">${tx.date}</span>
                            </td>
                            <td style="color:#f87171;">-$${tx.amount.toFixed(2)}</td>
                            <td><span class="${badgeClass}">${tx.type}</span></td>
                            <td>
                                <button class="btn-delete" onclick="deletePurchase(${tx.id})">Remove</button>
                            </td>
                        </tr>
                    `;
                });
        ...
        async function deletePurchase(id) {
            if (!confirm('Are you sure you want to remove this purchase and restore the funds to your balance?')) {
                return;
            }
...
class MealPlanHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        elif self.path == '/api/transaction/delete':
            success = engine.delete_transaction(payload['id'])
            if success:
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
